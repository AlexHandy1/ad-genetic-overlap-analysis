...
for (apoe in apoe_status) {
  ...
  print(paste("STEP 1: Load PRS Results prepared for meta analysis: ", apoe))
  ...
  for (strat in strats) {
    ...
    prs_results_file <- paste("protein_prs_",strat,apoe,"_res_for_meta_analysis.csv",sep="")

    prsResults <- read.csv(prs_results_file, header=T)
    ...
    print(resultsTable)
    output_file <- paste("protein_prs_",strat,apoe,"_meta_analysis_res.csv",sep="")
